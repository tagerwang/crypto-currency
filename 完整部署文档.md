...
7. [后续更新部署](#后续更新部署)
8. [HTTPS/SSL 配置（可选）](#httpssl-配置可选)
9. [故障排查](#故障排查)
...
├── mcp_http_server.py          # HTTP 包装器（REST API）
├── unified_server.py           # 统一 MCP 服务器（stdio）
├── requirements.txt            # Python 依赖列表
├── quick_deploy.sh             # 部署脚本
├── alpha_competitions.json     # Alpha 竞赛配置
...
1. **导入 MCP 函数**：
   - 直接导入 `binance_mcp` 模块和 `coingecko_mcp.py` 中的函数
...
```bash
# 上传整个模块
rsync -avz binance_mcp/ root@YOUR_SERVER_IP:/opt/mcp-crypto-api/binance_mcp/
...
如果你有域名并希望使用 HTTPS 加密连接，可以配置 SSL 证书。

### 方法 1：使用 Let's Encrypt（推荐）

**前提条件**：
- 拥有一个域名（如 `api.yourdomain.com`）
- 域名 DNS 已指向服务器 IP
...
### 方法 2：使用 DuckDNS 免费域名

如果没有域名，可以使用 DuckDNS 获取免费子域名：

```bash
# 使用项目提供的脚本
./setup_ssl_for_duckdns.sh
```

详细说明请参考 `SSL_SETUP_GUIDE.md`。

### 方法 3：使用隧道服务（开发测试）

如果只是临时测试或开发，可以使用 Cloudflare Tunnel 或 ngrok：
...
**注意**：隧道服务适合开发测试，生产环境建议使用正式域名和 SSL 证书。
...
- `mcp_http_server.py`：HTTP 包装器，将 MCP 工具暴露为 REST API
- `unified_server.py`：统一 MCP 服务器（stdio 模式，用于本地开发）
- `binance_mcp/`：币安 MCP 模块（模块化包）
  - `api.py`：币安 API 调用
  - `analysis.py`：技术分析功能
  - `alpha.py`：Alpha 空投管理
...
- `alpha_competitions.json`：Alpha 竞赛配置文件
...
curl http://YOUR_SERVER_IP/health
curl http://YOUR_SERVER_IP/binance/spot/price?symbol=BTC
curl http://YOUR_SERVER_IP/binance/alpha/active-competitions
curl http://YOUR_SERVER_IP/coingecko/trending

# 查看 Alpha 竞赛配置
ssh root@YOUR_SERVER_IP 'cat /opt/mcp-crypto-api/alpha_competitions.json'
```
...
**更新日志**：
- v1.1 (2025-01-20): 更新文件说明，添加 HTTPS/SSL 配置章节，补充 Alpha 空投相关内容
